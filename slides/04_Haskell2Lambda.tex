...
\begin{frame}{3 --- Stream-Kombinatoren}
    ...
    \begin{equation*}
        \{p^i | p\;\text{Primzahl}, 1 \leq i \leq n \}
    \end{equation*}
    ...
    \only<2>{
        ...
        \begin{figure}
            \begin{tabular}{c | c c c c c c c c c c c}
                i & \multicolumn{10}{c}{\texttt{map (\string^i) primes}} \\
                \hline
        ...
        \begin{itemize}
            \item \texttt{foldr} funktioniert auch mit unendlichen Listen\\
                  $\leadsto$ Aber weiß nicht, dass \texttt{primes}/\texttt{merge xs ys} monoton.
            \pause
            \item \texttt{foldl} funktioniert nur mit endlichen Listen\\
                  $\implies$ \texttt{foldl merge} über \texttt{map (\string^i) primes}
        \end{itemize}
    }
