...
\title{Tutorium 08: Parallelprogrammierung mit MPI}
% \subtitle{}
...
\begin{frame}{MPI: Grundgerüst}
  ...
  \begin{itemize}
    ...
    \item \texttt{MPI_COMM_WORLD}: Communicator, der alle Prozesse enthält
...
\begin{frame}{Aufgabe zu Scatter und Gather}
	...
	\begin{itemize}
		\item $N$: Prozessoranzahl (\texttt{MPI_Comm_size}), $x = 1000$
...
\begin{frame}{Allgather}
	\texttt{Allgather} ist die Verkettung von \texttt{Gather} und \texttt{Bcast}.
...
\begin{frame}{Reduce}
	...
	\begin{itemize}
		\item {\footnotesize \texttt{int MPI_Reduce(sendbuf, recvbuf, count, type, op, root, comm)}}
                \begin{itemize}
                  \item Beispiele für \texttt{op}: \texttt{MPI_SUM}, \texttt{MPI_PROD}, \texttt{MPI_MIN}, \texttt{MPI_MAX}, etc.
                \end{itemize}
...
\begin{frame}{Allreduce}
	...
	\begin{itemize}
		\item {\footnotesize \texttt{int MPI_Allgather(sendbuf, recvbuf, count, type, comm, op, comm)}}
                \item Wie bei \texttt{Allreduce}/\texttt{Alltoall}: Kein \texttt{root}-Parameter
  ...
  % TODO
	\begin{itemize}
		\item Im Campus-System könnt ihr euch bis zum 23.03. für die PP-Klausur anmelden
                ...
                \end{itemize}
		\item Ab Freitag kann man sich \href{https://campus.studium.kit.edu/renewal/payment.php}{Rückmelden} bis zum 15.02.
	\end{itemize}
\end{frame}
