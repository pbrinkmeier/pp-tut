...
\begin{frame}{Amdahlsches Gesetz}
  ...
  \only<3>{
    \begin{align*}
      p &= \frac{7s}{7s + 3s} = 0,7 \\
      T(n) &= (1 - p) + \frac{p}{n} = 0.3 + \frac{0,7}{n} \\
      S(4) &= \frac{T(1)}{T(4)} = \frac{1}{0,3 + 0,175} \approx 2,1
    \end{align*}
  }
\end{frame}
...
\begin{frame}{Lambdas}
  ...
  Ein Lambda ist hier Syntaxzucker für eine anonyme Klassendeklaration (gab es auch schon vor Java 8):
...
\begin{frame}{Das Interface \texttt{Function}}
  ...
  \begin{itemize}
    \item \text{java.util.function} enthält einige solcher Interfaces
